...
def allVR_noPD_348(request):
    ...
    if request.GET['matching'] == '0':
        ...
        df = pdQuery.decodeMachVR_noPD_levels_sorted_348(id_inspection)
        df = df.fillna('')
        #################################################################
        #EXPORT FULL INFO TO DEBUG
            # df.to_excel("resMergeWms.xlsx", sheet_name='ddp_dfwms_onCodeUnit-wmsProduct')
...
def testPage(request):
    print("inTestPage.")
    querys.connect()
    list_mails = ['test-7xurioe12@srv1.mail-tester.com']
    list_mails = ['miguel@smartcubik.com','miguel@kreometrology.com']
    utils.sendAlert(list_mails, "ScanBot Exception:" + "testEmail")

...
def all(request):
    ...
    query = "select count(distinct(codePos)) from inventorymaptbl where codePos not like '' and id_inspection=" + str(
        id_inspection)
...
def importWMS(request):
    data, description = querys.getWMSData(request.GET['id_inspection'])

    if request.method == "POST":
        print("myfile", request.POST['myfile'])
        querys.insertDATA(request.POST['myfile'])
